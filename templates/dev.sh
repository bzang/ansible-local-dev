...
HEREDOC
  ...
  def machine(args)
    if args.empty?
      ...
      output = run_with_timeout('docker-machine env {{ docker_machine_name }}', 2.0, 0.3)
      ...
      system('docker-machine create --driver virtualbox --engine-opt dns={{ dnsmasq_server_ip }} --engine-opt dns=8.8.8.8 --engine-opt dns=8.8.4.4 {{ docker_machine_name }}')
      system('docker-machine scp {{ dev_env_dir }}/local/docker/bootsync.sh dev:/tmp/bootsync.sh')
      system('docker-machine ssh {{ docker_machine_name }} "sudo mv /tmp/bootsync.sh /var/lib/boot2docker/bootsync.sh"')
      system('docker-machine restart dev')
    else
      args.push('{{ docker_machine_name }}') unless !args[1].nil? or ["ls", "help", "active", "create", "scp"].include?(args[0])
  ...
  def update(args)
    system('cd {{ dev_env_dir }} && git fetch && git reset --hard origin/master')
